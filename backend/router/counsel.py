...
from model.emotion_model.emotion_model import predict
...
def get_counsel(db = db_dependency, user_kakaotalk = str):
    ...
    counsel = db.query(Counsels).filter(Users.user_id == user.user_id).first()
    return counsel
...
def create_chatting(db: db_dependency, user: User, chatting: Chatting):
    try: 
        ...
        result = user.survey_question_one + user.survey_question_two + user.survey_question_three + user.survey_question_four + user.survey_question_five + chatting.chatting_user
...
def get_last_chatting(db: db_dependency, user_kakaotalk: str):
    try:
        ...
        return  db.query(Chattings).filter(Chattings.counsel_id == counsel.counsel_id).all()
    except:
