...
from database.database_model import Chatting, Users, User, Param_Chatting
...
def create_chatting_moodtracking(db:db_dependency, user: User, chatting: Param_Chatting):
    try:
        user = get_user(db, user.user_kakaotalk) 
        #model이 들어갈 부분 
        message_prompt =  "저는 지금 " + chatting.emotion_reason + " 때문에 "  + make_emotion_intensity(chatting.emotion_intensity) + transform_mood_epression(chatting.emotion_two)
        message_model = moodtracking_model(message_prompt)
        new_chatting = Chatting(
            user_id = user.user_id,
            message_prompt = message_prompt,
            message_model = message_model,
            emotion_reason = chatting.emotion_reason,
            emotion_one = chatting.emotion_one,
            emotion_two = chatting.emotion_two,
            emotion_intensity = chatting.emotion_intensity
        )
        db.add(new_chatting)
        db.commit()
...
def create_chatting(db: db_dependency, user: User, chatting: Param_Chatting):
    try: 
        user = get_user(db, user.user_kakaotalk)
        # model이 들어갈 부분 -------
        message_model = user.survey_question_one + user.survey_question_two + user.survey_question_three + user.survey_question_four + user.message_first
        #---------------------------
        new_chatting = Chatting(
            user_id = user.user_id,
            message_first = chatting.message_first,
            message_model = message_model,
        )
        db.add(new_chatting)
        db.commit()
